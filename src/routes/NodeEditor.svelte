<script lang="ts">
	...
	function addTag() {
		if (editedNode && newTagKey && newTagValue) {
			if (editedNode.data.tags[newTagKey]) {
				alert('A tag with this key already exists!');
				return;
			}
			editedNode.data.tags[newTagKey] = newTagValue;
			newTagKey = '';
			newTagValue = '';
	...
	function removeTag(key: string) {
		if (editedNode) {
			delete editedNode.data.tags[key];
			editedNode = { ...editedNode }; // Trigger reactivity
		}
	}
...
<Sheet.Root bind:open>
	...
	<Sheet.Content side="right" class="w-full sm:max-w-md">
		...
		<div class="grid gap-4 py-4">
			...
			{#if editedNode && editedNode.data.type}
				<div class="grid grid-cols-4 items-center gap-4">
					<Label for="nodeType" class="text-right">Node Type</Label>
					<Select
						bind:value={editedNode.data.type}
			...
			{/if}
			<Separator></Separator>
			{#if editedNode && editedNode.data.tags}
				<h3>Tags</h3>
				{#each Object.entries(editedNode.data.tags) as [key, value]}
					<div class="grid grid-cols-5 items-center gap-4">
						<Label for={`tag-${key}`} class="text-right col-span-2">{key}</Label>
						<Input
							id={`tag-${key}`}
							bind:value={editedNode.data.tags[key]}
							class="col-span-2"
						/>
						<Button on:click={() => removeTag(key)} variant="ghost"><CircleMinus class="h-4 w-4"></CircleMinus></Button>
					</div>
				{/each}
				<div class="grid grid-cols-5 items-center gap-4">
					...
					<Button on:click={addTag} variant="ghost"><CirclePlus class="h-4 w-4">></CirclePlus></Button>
				</div>
			{/if}
		</div>
		<Sheet.Footer>
